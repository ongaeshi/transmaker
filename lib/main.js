  ...
  pageMod.PageMod({
    ...
    onAttach: function(worker) {
      ...
      worker.port.on('translate', function (src) {
        // console.p(src.from);
        // console.p(gTranslateLang);
        var toLang =   (gTranslateLang.to != "")   ? gTranslateLang.to   : locale.getMylang(),
            fromLang = (gTranslateLang.from != "") ? gTranslateLang.from : locale.getFromLang(src.from);
  ...
  return panel.Panel({
    ...
    onMessage: function(msg) {
      switch (msg.kind) {
       case 'save':
        // console.p(msg);
        gTranslateLang.from = msg.from;
        gTranslateLang.to = msg.to;
        break;
       case 'close':
        this.hide();
        break;
